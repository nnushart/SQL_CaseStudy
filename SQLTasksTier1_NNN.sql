...
SELECT f.name, CONCAT(m.firstname, ' ', m.surname) AS member_name, f.guestcost*b.slots as cost
FROM Members AS m
		INNER JOIN Bookings as b
				ON m.memid = b.memid
...
SELECT f.name, CONCAT(m.firstname, ' ', m.surname) AS member_name, f.membercost*b.slots as cost
FROM Members AS m
		INNER JOIN Bookings as b
				ON m.memid = b.memid
		INNER JOIN Facilities as f
...
SELECT sq.name, sq.member_name, sq.cost
FROM (
    SELECT f.name, CONCAT(m.firstname, ' ', m.surname) AS member_name,
...
SELECT m.memid, m.surname, m.firstname,
      (r.firstname || ' ' || r.surname) AS recommending_member
FROM Members AS m
...
SELECT b.facid, f.name, strftime('%m', b.starttime) as month,
